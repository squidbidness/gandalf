...
#undef TEST_COMPONENTS
	...
	TEST( Vector, dot ) {
		EXPECT_EQ(
				4.0f,
				dot( V(0.0f, 1.0f, 2.0f, 3.0f), V(3.0f, 2.0f, 1.0f, 0.0f) )
				);
		EXPECT_EQ( 14.0, dot( V(1.0, 2.0, 3.0), V(1.0, 2.0, 3.0) ) );
		EXPECT_EQ( -2, dot( V(4, -2), V(-2, -3) ) );
		EXPECT_EQ( 14u, dot( V(4, 2), V(2, 3) ) );
	}
	...
	TEST( Vector, forEachComponent ) {
		...
		EXPECT_EQ( V(10.0f, 10.0f, 10.0f), v1 );

		EXPECT_EQ(
				V(10.0f, 40.0f, 90.0f, 160.0f),
				forEachComponent(
					[] ( auto I, auto const &a, auto const &b ) { return a[I] * b[I]; },
					V(1.0f, 2.0f, 3.0f, 4.0f),
					V(10.0f, 20.0f, 30.0f, 40.0f)
					)
				);
	}

	TEST( Vector, cross_canonical_bases ) {
		EXPECT_EQ( V(0, 0, 1), cross(V(1, 0, 0), V(0, 1, 0)) );
		EXPECT_EQ( V(0, -1, 0), cross(V(1, 0, 0), V(0, 0, 1)) );
		EXPECT_EQ( V(1, 0, 0), cross(V(0, 1, 0), V(0, 0, 1)) );
		EXPECT_EQ( V(0, 0, -1), cross(V(0, 1, 0), V(1, 0, 0)) );
		EXPECT_EQ( V(0, 1, 0), cross(V(0, 0, 1), V(1, 0, 0)) );
		EXPECT_EQ( V(-1, 0, 0), cross(V(0, 0, 1), V(0, 1, 0)) );
	}
}
